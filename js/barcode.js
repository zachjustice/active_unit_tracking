...
function init_barcode_handler( receive_barcode_input_callback ) {
    ...
    $(window).keypress(function(e) {
        ...
        // Pressed is initially set to false so we enter - this variable is
        // here to stop us setting a
        // timeout everytime a key is pressed. It is easy to see here that
        //  this timeout is set to give 
        // us 1 second before it resets everything back to normal.
        // If the keypresses have not matched 
        // the checks in the readBarcodeScanner function below then this is
        //  not a barcode
        if( pressed == false ) {
            // we set a timeout function that expires after 1 sec, once it
            // does it clears out a list 
            // of characters 
            setTimeout(function(){
                ...
                if (chars.length >= 10) {
                    ...
                    receive_barcode_input_callback( barcode );
                }
                chars = [];
                pressed = false;
            },500);
...
// this bit of code just ensures that if you have focus on the input which may
// be in a form
// that the carriage return does not cause your form to be submitted. Some
// scanners submit
